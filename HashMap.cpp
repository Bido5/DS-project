...
int HashMap::hashString(string value) { // calculate key value for the list which is the compressed value
    int sum = 0;
    for(int i =0; i<(int)value.size(); ++i){
        sum = sum * 26 + value[i] - 'A';
    }
...
string HashMap::searchByKey(int key) { //used for decompression
...
void HashMap::printHash() {
    for(int i=0; i<HASH_SIZE; i++){
        if(!table[i].empty()) cout<< i << ": "<<table[i]<< " " << endl;
    }
    cout<< counter<<endl;
}
